...
class ObjListCounter {
	...
	int lastptr;
		public:
	ObjListCounter() { memset(objlist, 0, sizeof(objlist)); lastptr = 0; };
	void Report() {
	...
	void AddObj(int id) { if (lastptr < MAXOBJS) objlist[lastptr++] = id; };
	void DelObj(int id) {
		for (int i = 0; i < lastptr; i++)
			if (objlist[i] == id)
				objlist[i] = 0;
...
int ObjectiveClass::Save(VU_BYTE **stream)
{
	...
	memcpy(*stream, &obj_data.last_repair, sizeof(CampaignTime));	*stream += sizeof(CampaignTime);
	memcpy(*stream, &obj_data.obj_flags, sizeof(ulong));			*stream += sizeof(ulong);
	memcpy(*stream, &obj_data.supply, sizeof(uchar));				*stream += sizeof(uchar);
	memcpy(*stream, &obj_data.fuel, sizeof(uchar));					*stream += sizeof(uchar);
	memcpy(*stream, &obj_data.losses, sizeof(uchar));				*stream += sizeof(uchar);
	len = ((static_data.class_data->Features*2)+7)/8;
	memcpy(*stream, &len, sizeof(uchar));							*stream += sizeof(uchar);
	memcpy(*stream, obj_data.fstatus, len);							*stream += len;
	memcpy(*stream, &obj_data.priority, sizeof(uchar));				*stream += sizeof(uchar);
	memcpy(*stream, &static_data.nameid, sizeof(short));			*stream += sizeof(short);
#ifdef CAMPTOOL
	if (gRenameIds)
		static_data.parent.num_ = RenameTable[static_data.parent.num_];
#endif
	memcpy(*stream, &static_data.parent, sizeof(VU_ID));			*stream += sizeof(VU_ID);
	memcpy(*stream, &static_data.first_owner, sizeof(Control));		*stream += sizeof(Control);
	memcpy(*stream, &static_data.links, sizeof(uchar));				*stream += sizeof(uchar);
...
#endif
		memcpy(*stream, &link_data[i], sizeof(CampObjectiveLinkDataType));	*stream += sizeof(CampObjectiveLinkDataType);
	}
	...
	if (static_data.radar_data)
	{
		...
		memcpy(*stream, static_data.radar_data, sizeof(RadarRangeClass));	*stream += sizeof(RadarRangeClass);
	}
	else
	{
...
int ObjectiveClass::Save (VU_BYTE **stream, int update)
{
	...
	memcpy(*stream, &obj_data.last_repair, sizeof(CampaignTime));	*stream += sizeof(CampaignTime);
	memcpy(*stream, &owner, sizeof(uchar));							*stream += sizeof(uchar);		
	memcpy(*stream, &obj_data.supply, sizeof(uchar));				*stream += sizeof(uchar);		
	memcpy(*stream, &obj_data.fuel, sizeof(uchar));					*stream += sizeof(uchar);		
	memcpy(*stream, &obj_data.losses, sizeof(uchar));				*stream += sizeof(uchar);		
	memcpy(*stream, &len, sizeof(uchar));							*stream += sizeof(uchar);		
...
void ObjectiveClass::SendDeaggregateData (VuTargetEntity *target)
{
	...
	memcpy(ddptr, &(tempId.creator_), sizeof(VU_SESSION_ID));					ddptr += sizeof(VU_SESSION_ID);
	tempId = FalconLocalSession->Id();
	memcpy(ddptr, &(tempId.creator_), sizeof(VU_SESSION_ID));	ddptr += sizeof(VU_SESSION_ID);
	memcpy(ddptr, &obj_data.obj_flags, sizeof(short));							ddptr += sizeof(short);
	memcpy(ddptr, &len, sizeof(uchar));											ddptr += sizeof(uchar);
	memcpy(ddptr, obj_data.fstatus, len);										ddptr += len;
	memcpy(ddptr, &static_data.class_data->Features, sizeof(uchar));			ddptr += sizeof(uchar);
...
int ObjectiveClass::Reaggregate (FalconSessionEntity*)
{
	...
	if (TheCampaign.IsOnline()){
		...
		memcpy(dataptr, &len, sizeof(uchar));							dataptr += sizeof(uchar);
		memcpy(dataptr, obj_data.fstatus, len);							dataptr += len;
		FalconSendMessage(msg,TRUE);
	}
...
void ObjectiveClass::ReaggregateFromData (VU_BYTE* data, long size)
{
	...
	// Get current status 
	uchar	len;
	memcpy(&len, data, sizeof(uchar));							data += sizeof(uchar);
	memcpy(obj_data.fstatus, data, len);						data += len;
...
int ObjectiveClass::ApplyDamage (FalconCampWeaponsFire *cwfm, uchar bonusToHit)
{
	...
	*gDamageStatusPtr = size;									gDamageStatusPtr++;
	memcpy(gDamageStatusPtr,obj_data.fstatus,size);				gDamageStatusPtr += size;
...
int ObjectiveClass::ApplyDamage (DamType d, int *str, int f, short flags)
{
	...
	while (*str > 0 && count < MAX_DAMAGE_TRIES)
	{
		...
		if (f >= static_data.class_data->Features || f < 0 || GetFeatureStatus(f) == VIS_DESTROYED || !GetFeatureID(f))
		{
			...
			{
				if (GetFeatureStatus(fid) != VIS_DESTROYED && GetFeatureClassData(GetFeatureID(fid))->DamageMod[d] > 0)
					f = fid;
			}
		...
		if (fc && fc->DamageMod[d] > 0)
		{
			...
			if (this_pass)
			{
				...
				*gDamageStatusPtr = (uchar)f;		gDamageStatusPtr++;
				*gDamageStatusPtr = (uchar)s;		gDamageStatusPtr++;
...
int ObjectiveClass::DecodeDamageData (uchar *data, Unit shooter, FalconDeathMessage *dtm)
{
	...
	if (!islocal)
	{
		...
		memcpy(obj_data.fstatus,data,size);				data += size;
	}
...
void ObjectiveClass::SetFeatureStatus (int f, int n, int from)
{
	...
	if (n == VIS_DESTROYED || n == VIS_REPAIRED)
	{
		if (from != f-1 && (FeatureEntryDataTable[static_data.class_data->FirstFeature+f].Flags & FEAT_PREV_CRIT))
			SetFeatureStatus (f-1, n, f);
		if (from != f+1 && (FeatureEntryDataTable[static_data.class_data->FirstFeature+f].Flags & FEAT_NEXT_CRIT))
			SetFeatureStatus (f+1, n, f);
	}
...
uchar ObjectiveClass::GetBestTarget (void)
{
	...
	{
		v = GetFeatureValue(i);
		if (v > bv && GetFeatureStatus(i) != VIS_DESTROYED)
		{
			bv = v;
			f = i;
		}
...
void SaveBaseObjectives(char* scenario)
{
	...
	// Now save
	{
		...
		{
			type = o->Type();
			memcpy(bufptr, &type, sizeof(short));			bufptr += sizeof(short);
			o->Save(&bufptr);
			// Clear dirty flag
			o->SetDelta(0);		
			o = GetNextObjective(&myit);
...
int BestRepairFeature (Objective o, int *hours)
{
	...
	for (f=0; f<o->GetTotalFeatures(); f++)
	{
		...
		if (s != VIS_NORMAL && s != VIS_REPAIRED)
		{
...
int BestTargetFeature (Objective o, uchar targeted[])
{
	...
	{
		s = o->GetFeatureStatus(f);
		if (s != VIS_DESTROYED && targeted && !targeted[f])
		{
...
void CaptureObjective(Objective co, Control who, Unit u)
{
	...
	if ((co->GetType() == TYPE_AIRBASE) || (co->GetType() == TYPE_AIRSTRIP)){
		...
		for (unit = (Unit) fit.GetFirst(); (unit != NULL); unit = (Unit) fit.GetNext()){
			FlightClass *f;
	...
	// Encode the data
	{
		...
		while (ent) 
		{
			if (
				...
				{
					vuid = o->Id();
					memcpy(buf, &vuid, sizeof(VU_ID));							buf += sizeof(VU_ID);		
					o->Save(&buf,1);
				}
			}
			ent = (CampEntity) myit.GetNext();
