...
uchar RoEData[ROE_NAVAL_BOMBARD+1][War+1] = {
	...
	{ ROE_NOT_ALLOWED,	ROE_NOT_ALLOWED,	ROE_NOT_ALLOWED,	ROE_NOT_ALLOWED,	ROE_ALLOWED,		ROE_ALLOWED		}};	// ROE_NAVAL_BOMBARD	
...
//#endif
	...
	if (gCampDataVersion < 41)
	{
		...
		else if (FalconLocalGame && FalconLocalGame->GetGameType() == game_TacticalEngagement)
		{
			if (who == 1)
				teamColor = 2;
		}
	...
	else if (FalconLocalGame && FalconLocalGame->GetGameType() == game_InstantAction)
	{
		if (who == 0)
			teamColor = 1;
		else if (who == 1)
...
int TeamClass::Save(VU_BYTE **stream)
{
	...
	memcpy(*stream, &who, sizeof(Team));								*stream += sizeof(Team);
	memcpy(*stream, &cteam, sizeof(Team));								*stream += sizeof(Team);
	memcpy(*stream, &flags, sizeof(short));								*stream += sizeof(short);
	memcpy(*stream, member, sizeof(uchar)*NUM_COUNS);					*stream += sizeof(uchar)*NUM_COUNS;
	memcpy(*stream, stance, sizeof(short)*NUM_TEAMS);					*stream += sizeof(short)*NUM_TEAMS;
	memcpy(*stream, &firstColonel, sizeof(short));						*stream += sizeof(short);
	memcpy(*stream, &firstCommander, sizeof(short));					*stream += sizeof(short);
	memcpy(*stream, &firstWingman, sizeof(short));						*stream += sizeof(short);
	memcpy(*stream, &lastWingman, sizeof(short));						*stream += sizeof(short);
	memcpy(*stream, &airExperience, sizeof(uchar));						*stream += sizeof(uchar);
	memcpy(*stream, &airDefenseExperience, sizeof(uchar));				*stream += sizeof(uchar);
	memcpy(*stream, &groundExperience, sizeof(uchar));					*stream += sizeof(uchar);
	memcpy(*stream, &navalExperience, sizeof(uchar));					*stream += sizeof(uchar);
	memcpy(*stream, &initiative, sizeof(short));						*stream += sizeof(short);
	memcpy(*stream, &supplyAvail, sizeof(ushort));						*stream += sizeof(ushort);
	memcpy(*stream, &fuelAvail, sizeof(ushort));						*stream += sizeof(ushort);
	memcpy(*stream, &replacementsAvail, sizeof(ushort));				*stream += sizeof(ushort);
	memcpy(*stream, &playerRating, sizeof(float));						*stream += sizeof(float);
	memcpy(*stream, &lastPlayerMission, sizeof(CampaignTime));			*stream += sizeof(CampaignTime);
	memcpy(*stream, &currentStats, sizeof(TeamStatusType));				*stream += sizeof(TeamStatusType);
	memcpy(*stream, &startStats, sizeof(TeamStatusType));				*stream += sizeof(TeamStatusType);
	memcpy(*stream, &reinforcement, sizeof(short));						*stream += sizeof(short);
	memcpy(*stream, bonusObjs, sizeof(VU_ID)*MAX_BONUSES);				*stream += sizeof(VU_ID)*MAX_BONUSES;
	memcpy(*stream, bonusTime, sizeof(CampaignTime)*MAX_BONUSES);		*stream += sizeof(CampaignTime)*MAX_BONUSES;
	memcpy(*stream, objtype_priority, sizeof(uchar)*MAX_TGTTYPE);		*stream += sizeof(uchar)*MAX_TGTTYPE;
	memcpy(*stream, unittype_priority, sizeof(uchar)*MAX_UNITTYPE);		*stream += sizeof(uchar)*MAX_UNITTYPE;
	memcpy(*stream, mission_priority, sizeof(uchar)*AMIS_OTHER);		*stream += sizeof(uchar)*AMIS_OTHER;
	memcpy(*stream, max_vehicle, sizeof(uchar)*4);						*stream += sizeof(uchar)*4;
	memcpy(*stream, &teamFlag, sizeof(uchar));							*stream += sizeof(uchar);
	memcpy(*stream, &teamColor, sizeof(uchar));							*stream += sizeof(uchar);
	memcpy(*stream, &equipment, sizeof(uchar));							*stream += sizeof(uchar);
	memcpy(*stream, name, sizeof(_TCHAR)*MAX_TEAM_NAME_SIZE);			*stream += sizeof(_TCHAR)*MAX_TEAM_NAME_SIZE;
	memcpy(*stream, teamMotto, sizeof(_TCHAR)*MAX_MOTTO_SIZE);			*stream += sizeof(_TCHAR)*MAX_MOTTO_SIZE;
	memcpy(*stream, &groundAction, sizeof(TeamGndActionType));			*stream += sizeof(TeamGndActionType);
	memcpy(*stream, &defensiveAirAction, sizeof(TeamAirActionType));	*stream += sizeof(TeamAirActionType);
	memcpy(*stream, &offensiveAirAction, sizeof(TeamAirActionType));	*stream += sizeof(TeamAirActionType);
	...
	memcpy(*stream, &tmpID, sizeof(VU_ID));								*stream += sizeof(VU_ID);
	if (gtm)
	{
		tmpID = gtm->Id();
	}
...
int TeamClass::Handle(VuFullUpdateEvent *event)
{
	...
	{
		if ((TeamInfo[i]) && (!(TeamInfo[i]->flags & TEAM_UPDATED)))
			return retval;
	}
...
void TeamClass::SelectAirActions (void)
{
	...
	if ((groundAction.actionType == GACTION_OFFENSIVE) && (action == AACTION_DCA) && (airRatio > 40)){
		action = AACTION_OCA;
	}
...
int SaveTeams (char* scenario)
{
	...
	i = 0;
	while (i < NUM_TEAMS){
		if (TeamInfo[i] && TeamInfo[i]->VuState() == VU_MEM_ACTIVE){
			teams++;
		}
		i++;
	}
...
{
if (TeamInfo[who]->bonusTime[i] > currtime && time > 0)
TeamInfo[who]->bonusTime[i] += time;
else if (TeamInfo[who]->bonusTime[i] > currtime && time < 0)
...
int GetPriority (MissionRequest mis)
{
	...
	if (!TeamInfo[mis->who]->GetMissionPriority (mis->mission))
		return -1;
	...
	{
		// KCK: Make sure we ignore any requests if the player has specified 0 priority
		if (!TeamInfo[mis->who]->GetObjTypePriority(e->GetType()))
			return -1;
	...
	else if (e->IsUnit() && e->GetDomain() == DOMAIN_LAND)
	{
		// KCK: Make sure we ignore any requests if the player has specified 0 priority
		if (!TeamInfo[mis->who]->GetUnitTypePriority(e->GetSType()))
			return -1;
...
void AddReinforcements (Team who, int inc)
{
	...
	while (u)
	{
		...
		if (u->GetTeam() == who && !u->Cargo() && u->GetUnitReinforcementLevel() <= TeamInfo[who]->GetReinforcement() && u->Parent())
		{
			added = 0;
			if (u->IsBrigade()){
...
void UpdateTeamStatistics(void)
{
	...
	for (i=0; i<NUM_TEAMS; i++)
	{
		if (TeamInfo[i]->flags & TEAM_ACTIVE)
		{
			...
			// A.S. 2001-12-09 
			if (!NewInitiativePoints) // The new procedure NewInitiativePointSetting does the job.
			{
				// same as old code 
				if (TeamInfo[i]->GetInitiative() < 40)
					TeamInfo[i]->AddInitiative ((INITIATIVE_LEAK_PER_HOUR * MIN_RECALCULATE_STATISTICS) / 60);
...
void InsertIntoSortedList (priority_structure *el)
{
	...
	tmp = PriorityList;
	while (tmp && tmp->average_score > el->average_score)
	{
		last = tmp;
		tmp = tmp->next;
	}
