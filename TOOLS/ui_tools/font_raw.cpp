...
void ExportToRaw(char *filename)
{
	...
	mem=new RGB24BIT[800*600];
	memset(mem,0,800*600*sizeof(RGB24BIT));
			...
			for(k=0;k<thefont->ByteWidth();k++)
			{
				...
				for(l=0;l<8;l++)
				{
					if(seg & 1)
					{
						if(rep == i)
						{
							mem[(y+j)*800+x+k*8+l].r=0xff;
							mem[(y+j)*800+x+k*8+l].g=0xff;
							mem[(y+j)*800+x+k*8+l].b=0xff;
						}
						else
						{
							mem[(y+j)*800+x+k*8+l].g=0xff;
	...
	if(fp)
	{
		fwrite(mem,800*600*sizeof(RGB24BIT),1,fp);
		fclose(fp);
	}
	else
