	...
	if (!((SimBaseClass*)entity_)->IsAwake()){
		...
		// predict
			...
			if (drsp > evsp){
				...
				d_drx_ = adjFactor * dx; d_dry_ = adjFactor * dy; d_drz_ = adjFactor * dz;
			}
			else {
				// under limit, no prob
				d_drx_ = dx; d_dry_ = dy; d_drz_ = dz;
			}
			...
			ct[0] = dryaw_; ct[1] = drpitch_; ct[2] = drroll_;
			et[0] = event->yaw_; et[1] = event->pitch_; et[2] = event->roll_;
			for (unsigned int i=0;i<3;++i){
				...
				if (dist > VU_PI){ dist -= VU_TWOPI; }
				else if (dist < -VU_PI){ dist += VU_TWOPI; }
			...
			d_dryaw_ = pdt[0]; d_drpitch_ = pdt[1]; d_drroll_ = pdt[2];
		}
