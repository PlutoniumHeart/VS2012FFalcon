...
//=================================
// Cobra - Externalize for testing
extern int g_nSfxLODCutoff;
extern int g_nSfxLODDistCutoff;
extern int g_nSfxLODTotCutoff;
extern bool g_bOldDustTrail; // Cobra - Use old dust trail sfx

int gSfxLODCutoff = g_nSfxLODCutoff;
...
SfxClass::SfxClass ( DrawableParticleSys *drawPartSys )
{
	...
	objTrail = NULL;
	//RV I-Hawk - RV new trails variables
	TrailNew = NULL;
	TrailIdNew = NULL;
	objTracer = NULL;
	objBSP = NULL;
					...
					FalconDamageMessage *damM )
{
	...
	secondaryCount = 1;
	secondaryInterval = 0.0f;
	secondaryTimer = SIM_ELAPSED_SEC + PRANDFloatPos() * 25.0f;
	travelDist = 0.0f;
					...
					float scaleSfx )
{
	...
	TryParticleEffect(); // if it succeeds, it changes the type
...
BOOL SfxClass::Exec(){
	...
	if ( type == SFX_SMOKING_FEATURE || type == SFX_STEAMING_FEATURE ){
		...
		if ( SIM_ELAPSED_SEC >= secondaryTimer ){
			// temporarily use the objBSP ptr
			...
			secondaryTimer = SIM_ELAPSED_SEC + 0.50f + 2.5F * groundZ;
...
SfxClass::Draw (void)
{
	...
	if ( (!(flags & SFX_F16CRASHLANDING) && !(flags & SFX_MOVES)) || (flags & SFX_TIMER_FLAG) )
	{
		...
		if ( baseObj )
		{
            // Pu239 This shit is generating problems n the free flight TE, comment it out and everything works fine.
