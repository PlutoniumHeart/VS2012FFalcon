			...
			float &range, float &radius, float &canSee)
{
	...
	{				
		...
		if (canSee > 0.8F)
		{
			...
			{
				// Actual LOS
				if (!OTWDriver.CheckLOS( platform, contact))
				{
...
void RadarDopplerClass::GMMode (void)
{
	...
	if (SimLibElapsedTime - lastFeatureUpdate > 500)
	{
		...
		// walk features first, features only show on GM display
		{
			...
			while(testFeature)
			{
				...
				else
				{
					if(canSee > 1.0f)
					{
						if(!testFeature->IsSetFELocalFlag(FELF_ON_PLAYERS_GM_CONTACT_LIST))
			...
			while(testFeature)
			{
				...
				else
				{
					if(canSee > 1.0f)
					{
						...
						if( gmtCanSee )
						{
							if(!testFeature->IsSetFELocalFlag(FELF_ON_PLAYERS_GMT_CONTACT_LIST))
							{
								GMList *newNode = new GMList (testFeature);
		...
		while(*next)
		{
			...
			if(!test->Object()->IsSetFELocalFlag(FELF_ON_PLAYERS_GM_CONTACT_LIST))
			{
		...
		while(*next)
		{
			...
			if(!test->Object()->IsSetFELocalFlag(FELF_ON_PLAYERS_GMT_CONTACT_LIST))
			{
				*next = test->next; // removes the node from the list
			    test->Release(); // self deleting
			}
...
//   cRangeSQ =  tmpX*tmpX + viewOffsetRel.y*viewOffsetRel.y;
	...
	cRangeSquaredGMScope = cRangeSQ;GetDOFValue(ComplexGearDOF[1]) == (af->GetAeroData(AeroDataSet::NosGearRng + 4) * DTR)
