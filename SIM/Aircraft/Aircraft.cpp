...
void AircraftClass::CleanupLocalData(){
	if (af){ delete (af); af = NULL; }
	if (mFaults){ delete mFaults; mFaults = NULL; }
	if (SimDriver.GetPlayerEntity() == this) {	//VWF
		gNavigationSys->DeleteMissionTacans();
	}
	if (Sms){ delete Sms; Sms = NULL; }	
	if (FCC){ delete FCC; FCC = NULL;}
}
...
float AircraftClass::GetA2GJDAMAlt(void)		{return af->auxaeroData->A2GJDAMAlt;}
float AircraftClass::GetA2GJSOWAlt(void)		{return af->auxaeroData->A2GJSOWAlt;}
float AircraftClass::GetA2GHarmAlt(void)		{return af->auxaeroData->A2GHarmAlt;}
float AircraftClass::GetA2GAGMAlt(void)			{return af->auxaeroData->A2GAGMAlt;}
float AircraftClass::GetA2GGBUAlt(void)			{return af->auxaeroData->A2GGBUAlt;};
float AircraftClass::GetA2GDumbHDAlt(void)		{return af->auxaeroData->A2GDumbHDAlt;}
float AircraftClass::GetA2GClusterAlt(void)		{return af->auxaeroData->A2GClusterAlt;}
float AircraftClass::GetA2GDumbLDAlt(void)		{return af->auxaeroData->A2GDumbLDAlt;}
float AircraftClass::GetA2GGenericBombAlt(void)		{return af->auxaeroData->A2GGenericBombAlt;}
float AircraftClass::GetA2GGunRocketAlt(void)	{return af->auxaeroData->A2GGunRocketAlt;}
float AircraftClass::GetA2GCameraAlt(void)		{return af->auxaeroData->A2GCameraAlt;}
float AircraftClass::GetA2GBombMissileAlt(void)	{return af->auxaeroData->A2GBombMissileAlt;}
...
void AircraftClass::WriteDirty(unsigned char **sAdd){
	...
	memcpy(stream, &dirty_aircraft, sizeof(dirty_aircraft)); stream += sizeof(dirty_aircraft);
	if (dirty_aircraft & DIRTY_ACSTATUS_BITS){
		memcpy(stream, &status_bits, sizeof(status_bits)); stream += sizeof(status_bits);
	}
