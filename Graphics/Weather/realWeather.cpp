...
void	RealWeather::SetDrawingOrder(float ZPosition)
{
	...
	switch(weatherCondition){
		...
		case	FAIR		:	TheDXEngine.DX2D_SetDrawOrder((DWORD*)&SunnyDrawOrder[Observer]);
								break;

		case	POOR		:	// POOR / INCLEMENT USE SAME TABLE
		case	INCLEMENT	:	TheDXEngine.DX2D_SetDrawOrder((DWORD*)&PoorDrawOrder[Observer]);
								break;
		...
		for(col = drawableCell; col < numCells-drawableCell; col++)
		{
			...
			&& col >= shadowCell && col < numCells-shadowCell)
			{
				...
				for(i = 0; i < 4; i++)
				{
					...
					clipFlag[i] |= GetHorizontalClipFlags(vp[i].x,vp[i].z);
					clipFlag[i] |= GetVerticalClipFlags(vp[i].y,vp[i].z);
				}
