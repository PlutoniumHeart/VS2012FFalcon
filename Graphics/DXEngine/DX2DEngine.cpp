...
void	CDXEngine::LoadTexture(char *FileName)
{
	...
	Ti->TuTv[0][0]=0.0f;		Ti->TuTv[0][1]=0.0f;
	Ti->TuTv[1][0]=1.0f;		Ti->TuTv[1][1]=0.0f;
	Ti->TuTv[2][0]=1.0f;		Ti->TuTv[2][1]=1.0f;
	Ti->TuTv[3][0]=0.0f;		Ti->TuTv[3][1]=1.0f;
	...
	while(1){
		...
		Ti->TuTv[0][0]=PosX;		Ti->TuTv[0][1]=PosY;
		Ti->TuTv[1][0]=PosX+SizeX;	Ti->TuTv[1][1]=PosY;
		Ti->TuTv[2][0]=PosX+SizeX;	Ti->TuTv[2][1]=PosY+SizeY;
		Ti->TuTv[3][0]=PosX;		Ti->TuTv[3][1]=PosY+SizeY;
	}
#endif
}
...
void	CDXEngine::DX2D_SetViewMode(void)
{
	...
	Flags.w=0xffff;	SetRenderState(Flags, Flags, DISABLE);
	// adjust the flags for the surface
	Flags.w=0;	Flags.b.Texture=Flags.b.Alpha=1;
...
void	CDXEngine::Draw3DPoint(D3DVECTOR *WorldPos, DWORD Color, bool Emissive, bool CameraSpace)
{
	...
	VPtr->dwColour=Color;
	if(Emissive) VPtr->dwSpecular=Color; else VPtr->dwSpecular=0;
...
void	CDXEngine::Draw3DLine(D3DVECTOR *WorldStart, D3DVECTOR *WorldEnd, DWORD ColorStart, DWORD ColorEnd, bool Emissive, bool CameraSpace)
{
	...
	if(Emissive) { VPtr->dwSpecular=ColorStart;(VPtr+1)->dwSpecular=ColorEnd; }
	else { VPtr->dwSpecular=0; (VPtr+1)->dwSpecular=0;}
...
void	CDXEngine::DX2D_SetupSquareCx(float y, float z)
{
	...
	vbb0.d3d.y=vbb3.d3d.y=-y; vbb1.d3d.y=vbb2.d3d.y=y;
	vbb0.d3d.z=vbb1.d3d.z=-z; vbb2.d3d.z=vbb3.d3d.z=z;
...
void	CDXEngine::FlushDynamicObjects(void)
{
	...
	Flags.w=0xffff;	SetRenderState(Flags, Flags, DISABLE);
	// adjust the flags for the surface
	Flags.w=0;	Flags.b.Alpha=Flags.b.Point=Flags.b.VColor=1;
	// set the engine status
	SetRenderState(Flags, Flags, ENABLE);
	// set the engine status
	m_pD3DD->SetRenderState(D3DRENDERSTATE_ALPHABLENDENABLE,TRUE);
