...
void CDXEngine::SetFogLevel(float FogLevel) {  m_FogLevel = m_LinearFog ? 1.0f: FogLevel; }
...
DWORD	CDXEngine::SetStencilMode(DWORD Stencil)
{
	...
	switch(Stencil){
		...
		case	STENCIL_OFF:		m_pD3DD->SetRenderState( D3DRENDERSTATE_STENCILFUNC, D3DCMP_ALWAYS);
									m_pD3DD->SetRenderState( D3DRENDERSTATE_STENCILPASS, D3DSTENCILOP_KEEP);
									m_pD3DD->SetRenderState( D3DRENDERSTATE_STENCILENABLE, FALSE);
									break;

		case	STENCIL_ON:			break;

		case	STENCIL_WRITE:		m_StencilRef++;
		...
		case	STENCIL_CHECK:		m_pD3DD->SetRenderState( D3DRENDERSTATE_STENCILFUNC, m_StencilRef?D3DCMP_GREATER:D3DCMP_ALWAYS );
									m_pD3DD->SetRenderState( D3DRENDERSTATE_STENCILMASK, 0xffffffff);
									m_pD3DD->SetRenderState( D3DRENDERSTATE_STENCILWRITEMASK, 0xffffffff);
									m_pD3DD->SetRenderState( D3DRENDERSTATE_STENCILREF, m_StencilRef);
									m_pD3DD->SetRenderState( D3DRENDERSTATE_STENCILPASS, D3DSTENCILOP_KEEP);
...
void	CDXEngine::SetViewMode(void)
{
	...
	switch (m_RenderState){
		...
		case	DX_DBS:	m_pD3DD->SetRenderState( D3DRENDERSTATE_TEXTUREFACTOR, NVG_T_FACTOR);
		...
		case	DX_OTW:
		default	:		m_pD3DD->SetTextureStageState(0,D3DTSS_COLORARG1,D3DTA_TEXTURE);
						m_pD3DD->SetTextureStageState(0,D3DTSS_COLORARG2,D3DTA_DIFFUSE);
						m_pD3DD->SetTextureStageState(0,D3DTSS_COLOROP,D3DTOP_MODULATE);
						m_pD3DD->SetTextureStageState(1,D3DTSS_COLOROP,D3DTOP_DISABLE);
						m_pD3DD->SetTextureStageState(2,D3DTSS_COLOROP,D3DTOP_DISABLE);
...
void CDXEngine::DrawSurface()
{
	...
	//if(NewFlags.StateFlags!=m_LastFlags.StateFlags){
		...
		/*if(EnabledFlags.StateFlags)*/ SetRenderState(NewFlags, NewFlags, ENABLE);
...
void CDXEngine::SWITCHManage()
{
	...
	while( m_NODE.DOF->SwitchNumber==SWNumber && (m_NODE.DOF->Type==SWITCH || m_NODE.DOF->Type==XSWITCH)){
		...
		if(Value&(1<<m_NODE.DOF->SwitchBranch)) { PushMatrix(&AppliedState);	return; }
#ifdef	EDIT_ENGINE
		m_SkipSwitch=true;
		m_DofLevel=1;
		return;
...
void CDXEngine::DrawObject(ObjectInstance *objInst, D3DXMATRIX *RotMatrix, const Ppoint *Pos, const float sx, const float sy, const float sz, const float scale, bool CameraSpace, DWORD LightOwner)
{
	...
	D3DVECTOR	p;
	bool		Visible=false;
	DWORD		Liter = 0; // Pu239 Initilize it!
	DxDbHeader	*Model;
	...
	if(m_RenderState == DX_DBS) { DrawBlip(objInst, RotMatrix, Pos, sx, sy, sz, scale, CameraSpace); return; }
	...
	else { if(!++LightID) LightID++; Liter=LightID; }
...
void CDXEngine::FlushObjects(void)
{
	...
	while(TheVbManager.GetDrawItem(&objInst, &LodID, &AppliedState, &Lited, &LightOwner, &m_FogLevel)){
		...
		D3DVECTOR pos;
		pos.x=AppliedState.m30; pos.y=AppliedState.m31; pos.z=AppliedState.m32;
...
void CDXEngine::DrawFrameSurfaces(NodeScannerType *NODE, float Alpha)
{
	...
	TheMaterial.emissive.a=Alpha; TheMaterial.emissive.g=TheMaterial.emissive.b=1.0f-SelectColor; TheMaterial.emissive.r=1.0f;
	TheMaterial.diffuse.a=Alpha; TheMaterial.diffuse.g=TheMaterial.diffuse.b=1.0f-SelectColor; TheMaterial.diffuse.r=1.0f;
	TheMaterial.ambient.a=Alpha; TheMaterial.ambient.g=TheMaterial.ambient.b=1.0f-SelectColor; TheMaterial.ambient.r=1.0f;
	TheMaterial.specular.a=Alpha; TheMaterial.specular.r=TheMaterial.specular.g=TheMaterial.specular.b=1.0f;;


	Mat2.emissive.a=Alpha*0.2f; Mat2.emissive.g=Mat2.emissive.b=1.0f-SelectColor; Mat2.emissive.r=SelectColor/2+0.5f;
	Mat2.diffuse.a=Alpha*0.2f; Mat2.diffuse.g=Mat2.diffuse.b=1.0f-SelectColor; Mat2.diffuse.r=SelectColor;
	Mat2.ambient.a=Alpha*0.2f; Mat2.ambient.g=Mat2.ambient.b=1.0f-SelectColor; Mat2.ambient.r=SelectColor;
	Mat2.specular.a=Alpha*0.2f; Mat2.specular.r=Mat2.specular.g=Mat2.specular.b=SelectColor;
