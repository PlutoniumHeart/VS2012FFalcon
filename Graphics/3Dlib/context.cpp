...
void ContextMPR::Render2DBitmap( int sX, int sY, int dX, int dY, int w, int h, int totalWidth, DWORD *pSrc, bool Fit)
{
	...
	try
	{
		...
		pVtx[0].x = (Float_t)dX;		pVtx[0].y = (Float_t)dY;		pVtx[0].u = (Float_t)0.0f;	pVtx[0].v = (Float_t)0.0f;
		pVtx[1].x = (Float_t)(dX + w);	pVtx[1].y = (Float_t)dY;		pVtx[1].u = (Float_t)1.0f;	pVtx[1].v = (Float_t)0.0f;
		pVtx[2].x = (Float_t)(dX + w);	pVtx[2].y = (Float_t)(dY + h);	pVtx[2].u = (Float_t)1.0f;	pVtx[2].v = (Float_t)1.0f;
		pVtx[3].x = (Float_t)dX;		pVtx[3].y = (Float_t)(dY + h);	pVtx[3].u = (Float_t)0.0f;	pVtx[3].v = (Float_t)1.0f;
		...
		for(int i = 0; i < poly->nVerts; i++)
		{
			...
			else if (OTWDriver.GetOTWDisplayMode() == OTWDriverClass::Mode3DCockpit)
			{
				...
				else
				{
					...
					{
						src = p3DpitLolite;
						pVtx->color = (src&0xFF000000)+((src&0x00FF0000)>>16)+(src&0x0000FF00)+((src&0x000000FF)<<16);
					}
					else
					{
						src = p3DpitHilite;
