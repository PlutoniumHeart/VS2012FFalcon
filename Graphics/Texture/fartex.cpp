...
void FarTexDB::SetLightLevel(void)
{
	...
	if( DisplayOptions.m_texMode != DisplayOptionsClass::TEX_MODE_DDS )
	{
		...
		tmpR = *from * 1.0f; from++;	// Red
		tmpG = *from * 1.0f; from++;	// Green
		tmpB = *from * 1.0f; from++;	// Blue
		from++; // Alpha
		...
		*to = static_cast<BYTE>(FloatToInt32(tmpR * lightColor.r)); to++;
		*to = static_cast<BYTE>(FloatToInt32(tmpG * lightColor.g)); to++;
		*to = static_cast<BYTE>(FloatToInt32(tmpB * lightColor.b)); to++;
		to++; // Alpha
...
void FarTexDB::Request(TextureID texID)
{
    ...
    ShiAssert(texID < (DWORD) texCount); // Pu239
...
void FarTexDB::Release(TextureID texID)
{
    ...
    // Pu239 Fucking idiots mixing DWORD with WORD
	ShiAssert(texID >= (DWORD) 0);
...
void FarTexDB::Load(DWORD offset, bool forceNoDDS)
{
    ...
    ShiAssert(offset < (DWORD) texCount); // Pu239
