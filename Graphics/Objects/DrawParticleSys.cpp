...
/**** Static class data ***/
DXContext *DrawableParticleSys::psContext = NULL;
...
int DrawableParticleSys::GetNameId(char *name)
{
    ...
    for (l = 0; l < PPNCount; l++)
    {
        ...
        if (PPN[l] && PS_PPN[(DWORD)PPN[l]].name && stricmp(name, PS_PPN[(DWORD)PPN[l]].name) == 0) 
            return l;
    ...
    while (ptr != PS_NOPTR)
    {
        ...
        if (Part.Cluster)
        {
            ...
            if (!LightIn)
            {
                ...
                if (!Cluster.In)
                    if (!TheDXEngine.DX2D_GetVisibility((D3DXVECTOR3*) &Part.pos, (ppn.EmitLight) ? size : LightRadius)) goto Skip;
...
void  DrawableParticleSys::PS_EmitterRun(void)
{
    ...
    while (ptr != PS_NOPTR)
    {
        ...
        if (!Alive) ptr = PS_RemoveItem(PS_EMITTERS_IDX, ptr, LastPtr);
...
{
    PS_AddParticle((int)PPN[ID]->id, Pos, Vel); //Pu239 Are you fucking kidding me?? Passing a memory address as an ID?
}
...
TRAIL_HANDLE DrawableParticleSys::PS_AddTrail(int ID, Tpoint *Pos, PS_PTR OWNER, bool Run, float AlphaCx, float SizeCx)
{
    ...
    if (!TrailsHandle) TrailsHandle = 1;
...
void DrawableParticleSys::PS_TrailRun(void)
{
    ...
    while (ptr != PS_NOPTR)
    {
        ...
        if (!PS_ElapsedTime) Trail.Updated = false;
...
void DrawableParticleSys::PS_SubTrailRun(TrailSubPartType *Trail, D3DXVECTOR3 &Origin, DWORD &Entry, DWORD &Exit, DWORD Elements, PS_PTR tpn)
{
    ...
    while (!TrailCompleted /*&& PS_SubTrails<14200*/)
    {
        ...
        if (TrailValid)
        {
            ...
            if (!TheDXEngine.DX2D_GetVisibility((D3DXVECTOR3*)&XMMPos.d3d, SegmentSize / 2.0f, CAMERA_VERTICES))
            {
                RecalcP1 = true;
                goto Skip;
            }
            ...
            */
            ...
            if (!SegmentStep) SegmentStep = Size / 2.0f + 0.5f; //min( Size / 2.0f + 0.5f, max( RED_Lenght3D(&S0) / FragRadius * 10, 1.0f));
            ...
            while (SegmentDone < SegmentSize)
            {
                ...
                if (ST_Distance >= FragMixOut)
                {
                    ...
                    if (RecalcP1)
                    {
                        ...
                        if (!EntrySegment) LastSegAlpha = LastColor.a;
...
#define On(s) if(stricmp(com,s)==0)
    ...
    // Pu239 This is fucking stupid!
    //for (int c = 0; c < l; c++)
    //{
    //    PPN[PS_PPN[c].id] = (ParticleParamNode *)c;
    //}
    // Pu239 I am guessing what the fuck was in this monkey's mind, when it was writing those crap above!
    for(int i=0;i<l;i++)
    {
        PPN[i] = new ParticleParamNode;
        PPN[i]->id = 0;
    }

    for(int i=0;i<l;i++)
    {
        if(PS_PPN[i].id>l || PS_PPN[i].id<0)
            continue;
        PPN[PS_PPN[i].id]->id = i;
    }
    ...
    for (DWORD a = 0; a < LastTPN; a++)
    {
        ...
        while (Table->Name[0])
        {
            ...
            if (!strcmp(PS_TPN[a].Name, Table->Name))
            {
    ...
    for (DWORD c = 0; c < MAX_PARTICLE_PARAMETERS; c++)
    {
        ...
        if (PS_PPN[c].TrailName[0])
        {
            ...
            for (DWORD a = 0; a < LastTPN; a++)
            {
                ...
                if (!strcmp(PS_TPN[a].Name, PS_PPN[c].TrailName)) 
                    PS_PPN[c].trailId = a;
            }
        }
    }
