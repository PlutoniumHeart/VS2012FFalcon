...
typedef enum {	PSD_SPHERE = 0, PSD_PLANE=1, PSD_BOX=2, PSD_BLOB=3, PSD_CYLINDER=4, PSD_CONE=5, PSD_TRIANGLE=6, PSD_RECTANGLE=7,
				PSD_DISC=8,	PSD_LINE=9,	PSD_POINT=10 }  PSDomainEnum;
...
public:
	...
	{
		float		 param[9];
		struct { Point pos, size; } sphere;
		struct { float a,b,c,d;	} plane;
		struct { Point cornerA, cornerB; } box;
		struct { Point pos, size; float  deviation;	} blob;
	};
	void Parse(void);
};
