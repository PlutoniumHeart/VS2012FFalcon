...
UI_HANDLE C_Parser::OpenArtFile(char *filename, const char *thrdir, const char *maindir, int hirescapable){
	...
	// try theater first
	strcpy(filebuf, thrdir); // Falcon thr root dir
	if (hirescapable) {
...
#define NIGHTFALCON_UI 1
	...
	strcat(filebuf,filename);
	ifp=UI_OPEN(filebuf,"rb");
	if (ifp != NULL){ return ifp; } // got the main one

	// try main dir
	strcpy(filebuf,maindir); // Falcon main root dir
	if (hirescapable) {
#if NIGHTFALCON_UI
		strcat(filebuf,"\\art");
#else
			...
			case SECTION_FINDPARAMS:
				...
				while(!Finished)
				{
					...
					while(!Found && !Done && !Finished)
					{
						switch(script_[Idx_])
						{
							case ' ': case ',': case 0x09: case 0x0a: case 0x0d:
								Idx_++;
								break;
							case '[':
								Finished=1;
